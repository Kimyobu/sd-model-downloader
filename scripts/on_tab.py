...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as ui_component:
        with gr.Row():
            ...
            model_url = gr.Textbox(label='Model Url',placeholder="Place model download url here (Not a url of page of model card)")
            submit = gr.Button('Download').style(full_width=False)

        with gr.Row():
            folder = gr.Textbox(label='Directory Path',value='embeddings', placeholder='Type your directory here')
            url = gr.Textbox(label='Download URL',placeholder="Place model download url here (Not a url of page of model card)")
...
def sub1(model_type, model_url):
    ...
    model_path = f'{scripts.os.getcwd()}/{model_type}'
    if model_type == 'ControlNet':
        model_path = f'{scripts.os.getcwd()}/extensions/sd-we'+'bui-controlnet/models'
    try:
        scripts.os.system(f'aria2c --console-log-level=error -c -x 16 -s 16 -k 1M {model_url} -d {model_path}')
...
def sub2(folder, url):
    ...
    try:
        scripts.os.system(f'aria2c --console-log-level=error -c -x 16 -s 16 -k 1M {url} -d {folder}')
